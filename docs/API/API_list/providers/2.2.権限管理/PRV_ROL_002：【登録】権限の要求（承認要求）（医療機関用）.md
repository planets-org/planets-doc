...
| 8   | 検索条件                  | searchCriteria                 |     |     |  ○  |     |     |     | -     | string | -        | OpenFRUCtoSへの検索条件<br/>※現時点では「文書所有者ID」「医療機関ID」「文書キー」「文書タイプ」のいずれかが指定される                                                                                                          |
...
| 3   | 検索条件                  | searchCriteria                 |     |     |  ○  |     |     |     | -     | string | -        | FRUCtoSへの検索条件                                                                                                          |
| 4   | 演算子                    | operator                       |     |     |  ○  |     |     |     | -     | string | -        | "検索条件 に対して値をどうチェックするか<br/>01:＝ ※ 現時点では「＝」のみ"                                                      |       |
| 5   | 値                        | value                          |     |     |  ○  |     |     |     | -     | string | -        | FRUCtoSへの検索値                                                                                                         |
