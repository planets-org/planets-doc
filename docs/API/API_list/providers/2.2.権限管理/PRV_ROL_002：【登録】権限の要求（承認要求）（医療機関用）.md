...
| 2   | 権限要求リスト            | permissionApproval      | 1 | ○     | array                | -        | -     | -      | -      | 制御文字以外 | -        | -        |                                                                                                                                  | 
| 3   | 許可可能者 ID（医療機関） | allowableOrganizationId | 2 | -      | string               | ○       | 10    | -      | -      | 以下の文字と記号のみ可<br/>・a-zA-Z0-9<br/>・記号[・-_.!*'()] |          | | 組織単位で設定する際は必須<br/>入力パターンに関しては「[補足：許可可能者IDの入力パターン](#補足許可可能者idの入力パターン)」を参照                                                 | 
| 4   | 許可可能者 ID（診療科）   | allowableDepartmentId   | 2 | -      | string               | ○       | 1000 | -      | -      | 以下の文字と記号のみ可<br/>・a-zA-Z0-9<br/>・記号[・-_.!*'()] |          |          | 組織単位で設定する際に診療科まで許可者を絞る際は設定<br/>入力パターンに関しては「[補足：許可可能者IDの入力パターン](#補足許可可能者IDの入力パターン)」を参照                                           | 
| 5   | 許可可能者 ID（個人）     | allowablePersonalId     | 2 | -      | string               | ○       | 36    | -      | -      | 以下の文字と記号のみ可<br/>・a-zA-Z0-9<br/>・記号[・-_.!*'()] |          |          | 個人単位で設定する際は必須<br/>入力パターンに関しては「[補足：許可可能者IDの入力パターン](#補足許可可能者IDの入力パターン)」を参照                                                   | 
| 6   | 権限リスト                | permissionList          | 1 | ○     | array                | -        | -     | -      | -      |              |          |          |                                                                                                                                  | 
| 7   | 権限保持対象区分          | classification          | 2 | -      | string | -        | -     | -      | -      |-             |          |          | 1:個人、2:組織                                                                                                                   | 
| 8   | 許可者 ID                 | permissionId            | 2 | -      | string               | -        | 36    | -      | -      |              |          |          | 【権限保持対象区分が1:個人の場合】<br/>付与対象者の PLAT 共通 ID もしくはスタッフ ID<br/>【権限保持対象区分が2:組織の場合】<br/>付与対象の医療機関ID                                 | 
...
| :-: | :-----: | :----: | :--: | ----------------- | -------------- |
| 1   | 〇      | 〇     | 〇   | スタッフID         | 診療科に所属しているスタッフ個人を指定 |
| 2   | 〇      | 〇     | ×    | -                 | 診療科を指定 |
