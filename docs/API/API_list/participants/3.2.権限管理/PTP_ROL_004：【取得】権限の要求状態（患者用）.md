...
| :---------- | :------------------------------- | :------------ | :-------------------------------------------------------------------------- |
| PTP_ROL_004 | 【取得】権限の要求状態（患者用） | GET           | {applicationPath}/participants/permission/requests/{permissionManagementId} |
...
クエリパラメータとパスパラメータが指定されていない場合は、自身が要求している権限の一覧を返却する。
...
```
　　{applicationPath}/participants/permission/requests/c42e5ca3-fb57-4e5a-9b75-a9b08aeddda4
```
...
| 23  | 権限保持者ID              | permissionId                   |     |  ○  |     |     |     |     | -     | string | -        | リクエストで設定した値と同一値                                                                                                  |
